...
std::optional<VersionChange> SelectionManager::Deselect(GameGrid &grid) {
  const auto retValue = [this, &grid]() {
    ...
    return std::optional<VersionChange>{{.Action = SelectionAction::Deselect,
                                         .SelectionBounds = SelectionBounds(),
...
std::optional<VersionChange> SelectionManager::Rotate(bool clockwise) {
  ...
  m_SentinelSelection = *m_AnchorSelection +
                        Vec2{m_Selected->Height() - 1, m_Selected->Width() - 1};
...
SelectionManager::Load(const std::filesystem::path &filePath) {
  ...
  m_SentinelSelection =
      result->Offset + Vec2{m_Selected->Width() - 1, m_Selected->Height() - 1};
...
bool SelectionManager::Save(const GameGrid &grid,
  ...
  return RLEEncoder::WriteRegion(*m_Selected, Rect{{0, 0}, m_Selected->Size()},
                                 filePath);
}
...
void SelectionManager::RestoreGridVersion(EditorAction undoRedo, GameGrid &grid,
  ...
  const auto &deletions = undoRedo == EditorAction::Undo ? change.CellsInserted
                                                         : change.CellsDeleted;
...
Vec2 SelectionManager::RotatePoint(Vec2F center, Vec2F point, bool clockwise) {
  auto offset =
      Vec2F{static_cast<float>(point.X), static_cast<float>(point.Y)} - center;
  auto rotated =

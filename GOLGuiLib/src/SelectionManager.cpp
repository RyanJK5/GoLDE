...
namespace gol
{
    SelectionUpdateResult SelectionManager::UpdateSelectionArea(GameGrid& grid, Vec2 gridPos)
    {
    ...
    std::optional<VersionChange> SelectionManager::InsertNoise(Rect selectionBounds, float density)
    {
    ...
    std::expected<VersionChange, std::string> SelectionManager::Load(const std::filesystem::path& filePath)
    {
    ...
    bool SelectionManager::Save(const GameGrid& grid, const std::filesystem::path& filePath) const
    {
        if (!m_Selected)
        {
    ...
    std::optional<VersionChange> SelectionManager::HandleAction(SelectionAction action, GameGrid& grid, int32_t nudgeSize)
    {
        switch (action)
        {
    ...
    void SelectionManager::HandleVersionChange(EditorAction undoRedo, GameGrid& grid, const VersionChange& change)
    {
        if (!change.Action)
        {
    ...
    void SelectionManager::RestoreGridVersion(EditorAction undoRedo, GameGrid& grid, const VersionChange& change)
    {
    ...
    int64_t SelectionManager::SelectedPopulation() const
    {
        return m_Selected ? m_Selected->Population() : 0;
    }
    ...
    bool SelectionManager::CanDrawGrid() const
    {

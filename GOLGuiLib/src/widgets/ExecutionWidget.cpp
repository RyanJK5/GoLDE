...
gol::Size2F gol::ResetButton::Dimensions() const {
    return {ImGui::GetContentRegionAvail().x / 3.f,
            ActionButton::DefaultButtonHeight};
}
std::string gol::ResetButton::Label(const EditorResult &) const {
    return ICON_FA_STOP;
}
bool gol::ResetButton::Enabled(const EditorResult &state) const {
    return state.State == SimulationState::Simulation ||
           state.State == SimulationState::Paused ||
           state.State == SimulationState::Stepping;
}

gol::Size2F gol::RestartButton::Dimensions() const {
    return {ImGui::GetContentRegionAvail().x / 2.f,
            ActionButton::DefaultButtonHeight};
}
std::string gol::RestartButton::Label(const EditorResult &) const {
    return ICON_FA_REPEAT;
}
bool gol::RestartButton::Enabled(const EditorResult &state) const {
    return state.State == SimulationState::Simulation ||
           state.State == SimulationState::Paused;
}

gol::Size2F gol::ClearButton::Dimensions() const {
    return {ImGui::GetContentRegionAvail().x,
            ActionButton::DefaultButtonHeight};
}
std::string gol::ClearButton::Label(const EditorResult &) const {
    return ICON_FA_TRASH;
}
bool gol::ClearButton::Enabled(const EditorResult &state) const {
    return state.State != SimulationState::Empty;
}

gol::SimulationControlResult
gol::ExecutionWidget::UpdateImpl(const EditorResult &state) {

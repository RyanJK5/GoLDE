...
namespace gol
{
    Size2F GenerateNoiseButton::Dimensions() const { return { ImGui::GetContentRegionAvail().x, ActionButton::DefaultButtonHeight }; }
    
    std::string GenerateNoiseButton::Label(const EditorResult&) const { return "Generate Noise"; }
    
    bool GenerateNoiseButton::Enabled(const EditorResult& state) const 
    { 
        return (state.State == SimulationState::Empty || state.State == SimulationState::Paint) && state.SelectionActive;
    }

...
namespace gol
{
    ...
    protected:
        ...
        virtual std::string Label(const EditorResult&) const override final { return "Apply"; }
        virtual bool Enabled(const EditorResult& state) const final { return state.State == SimulationState::Paint || state.State == SimulationState::Empty; }
    };

...
void StepWidget::SetStepCount(int64_t stepCount) {
  ...
  auto [end, error] =
      std::to_chars(m_InputText.Data, m_InputText.Data + m_InputText.Length + 1,
...
SimulationControlResult StepWidget::UpdateImpl(const EditorResult &state) {
  ...
  auto retValue = SimulationControlResult{.Action = result.Action,
  ...
  if (algo == LifeAlgorithm::HashLife)
    ...
    ImGui::SetItemTooltip("SparseLife is a straightforward algorithm that "

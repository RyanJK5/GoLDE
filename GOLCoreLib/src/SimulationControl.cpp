...
namespace gol
{
    ...
    void ButtonlessShortcuts::SetShortcuts(const ShortcutMap& shortcuts) {
        Shortcuts = {
            {{SelectionAction::NudgeLeft,
              shortcuts.at(SelectionAction::NudgeLeft) | KeyShortcut::RepeatableMapChordsToVector},
             {SelectionAction::NudgeRight,
    ...
    {
        auto lastKnownWriteTime = std::filesystem::last_write_time(m_ShortcutConfigPath);
    ...
    SimulationControlResult
    SimulationControl::Update(const EditorResult &state) {
        ...
        if (m_UpdateShortcuts.exchange(false, std::memory_order_relaxed)) {
            const auto fileInfo = ConfigLoader::LoadYAML<ImVec4>(m_ShortcutConfigPath);

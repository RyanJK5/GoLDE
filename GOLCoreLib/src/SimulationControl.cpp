...
gol::ButtonlessShortcuts::Update(const EditorResult &state) {
...
gol::SimulationControl::SimulationControl(
    const StyleLoader::StyleInfo<ImVec4> &fileInfo)
    : m_ButtonlessShortcuts(fileInfo.Shortcuts.at(SelectionAction::NudgeLeft),
      ...
      m_StepWidget(fileInfo.Shortcuts.at(GameAction::Step)), m_DelayWidget(),
      m_NoiseWidget(fileInfo.Shortcuts.at(EditorAction::GenerateNoise)) {}

void gol::SimulationControl::FillResults(
...
gol::SimulationControlResult
gol::SimulationControl::Update(const EditorResult &state) {

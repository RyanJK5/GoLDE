...
void SimulationEditor::DrawHashLifeData(const HashQuadtree &quadtree,
  ...
  const auto topLeftWorld = m_Graphics.Camera.ScreenToWorldPos(
      Vec2F{static_cast<float>(viewBounds.X), static_cast<float>(viewBounds.Y)},
      viewBounds);
...
SimulationEditor::SimulationUpdate(const GraphicsHandlerArgs &args) {
  ...
  if (std::holds_alternative<std::reference_wrapper<const LifeHashSet>>(data))
    m_Graphics.DrawGrid(
        {0, 0}, std::get<std::reference_wrapper<const LifeHashSet>>(data).get(),
        args);
  else
    DrawHashLifeData(
        std::get<std::reference_wrapper<const HashQuadtree>>(data).get(), args);
...
SimulationEditor::DisplaySimulation(bool grabFocus) {
  auto label = std::format(
      "{}{}###Simulation{}",
      m_CurrentFilePath.empty() ? "(untitled)"
  ...
  ImGui::Text("%s",
              std::format(std::locale(""), "Population: {:L}",
...
void SimulationEditor::UpdateVersion(const SimulationControlResult &args) {
  ...
  auto versionChanges = action == EditorAction::Undo ? m_VersionManager.Undo()
...
void SimulationEditor::UpdateMouseState(Vec2 gridPos) {
  ...
  if (ImGui::IsMouseDown(ImGuiMouseButton_Left) &&
    ...
    if (m_EditorMode == EditorMode::None ||
        m_EditorMode == EditorMode::Select) {
      m_EditorMode = *m_Grid.Get(gridPos.X, gridPos.Y) ? EditorMode::Delete
                                                       : EditorMode::Insert;
      m_VersionManager.BeginPaintChange(gridPos,

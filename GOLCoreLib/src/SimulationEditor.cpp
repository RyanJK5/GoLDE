...
namespace gol
{
    SimulationEditor::SimulationEditor(uint32_t id, const std::filesystem::path& path, Size2 windowSize, Size2 gridSize)
        : m_EditorID(id)
        ...
        , m_PasteWarning("Paste Warning", std::bind_front(&SimulationEditor::PasteWarnUpdated, this))
        , m_FileErrorWindow("File Error", [](auto) {})
        , m_SaveWarning("Save Warning", [](auto) {})
    { }   

    bool SimulationEditor::IsSaved() const
    {
    ...
    EditorResult SimulationEditor::Update(std::optional<bool> activeOverride, const SimulationControlResult& controlArgs, const PresetSelectionResult& presetArgs)
    {
    ...
    void SimulationEditor::DrawHashLifeData(const HashQuadtree& quadtree, const GraphicsHandlerArgs& args)
    {
    ...
    SimulationState SimulationEditor::SimulationUpdate(const GraphicsHandlerArgs& args)
    {
    ...
    SimulationEditor::DisplayResult SimulationEditor::DisplaySimulation(bool grabFocus)
    {
    ...
    Rect SimulationEditor::ViewportBounds() const
    {
        return WindowBounds();
    }
    ...
    SimulationState SimulationEditor::UpdateState(const SimulationControlResult& result)
    {
        ...
        if (auto* action = std::get_if<EditorAction>(&*result.Action))
        {
            ...
            case SaveAsNew:
                ...
                {
                    m_SaveWarning.SetCallback([this, result](PopupWindowState state)
                    {
    ...
    SimulationState SimulationEditor::ResizeGrid(const SimulationControlResult& result)
    {
